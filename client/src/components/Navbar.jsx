...
const Navbar = () => {
  ...
  return (
    <div className="flex md:flex-row flex-col items-center justify-between mb-[35px] gap-6">
      ...
      <div className="sm:hidden flex w-full justify-between items-center relative">
        ...
        <div
          className={`absolute top-[60px] right-0 left-0 bg-[#1c1c24] z-50 shadow-secondary py-4 ${
            !toggleDrawer ? "-translate-y-[100vh]" : "translate-y-0"
          } transition-all duration-700`}
        >
          <ul className="mb-4">
            {/* BUG FIX: userData is {} when logged out (not null), so userData.type
              is undefined. Fall back to customerNavlinks for unauthenticated users. */}
            {(userData?.type === "user"
              ? customerNavlinks
