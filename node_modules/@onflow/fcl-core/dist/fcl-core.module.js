...
// Use "1.30.2" if available (from build), otherwise use a default for development
 ...
 */
async function deriveCadenceByNetwork100({
  network,
  template
}) {
 ...
 * @returns Promise that resolves with the derived Cadence code
 */
async function deriveCadenceByNetwork110({
  network,
  template
}) {
 ...
 * @returns Promise that resolves with the derived Cadence code
 */
async function deriveCadenceByNetwork({
  network,
  template
}) {
 ...
 */
async function fetchChainId(context, opts = {}) {
...
function createGetChainId(context) {
   ...
   */
  async function getChainId(opts = {}) {
...
async function configLens(context, regex) {
  return Object.fromEntries(Object.entries(await context.config.where(regex)).map(([key, value]) => [key.replace(regex, ""), value]));
}
...
const makeHandlers$1 = (opts = {}) => ({
  ...
  [SUBSCRIBE]: (ctx, letter) => {
    ...
    ctx.send(letter.from, UPDATED$2, {
      ...ctx.all()
    });
  },
  ...
  [POLL]: async ctx => {
    ...
    if (!isSealed(tx)) poll();
    if (isDiff(prevTx, tx)) ctx.broadcast(UPDATED$2, {
      ...tx
    });
    ctx.merge(tx);
  }
});
const spawnTransaction = (opts = {}) => transactionId => {
 ...
 */
function transaction$1(transactionId, opts = {
  ...
  const isScheduledTxId = isScheduledTransactionId$1(scopedId);
...
function createTransaction(context) {
   ...
   * )
   */
  function transaction(transactionId, opts = {
    txNotFoundTimeout: 12500,
    pollRate: 1000
  }) {
    // Validate transactionId format
...
function createEvents(context) {
  ...
  function events(filterOrType) {
    ...
    return {
      subscribe: (onData, onError = error => {
...
const getServiceRegistry = () => {
  if (!getIsServiceRegistryInitialized()) {
    console.warn("Registry is not initialized, it will be initialized with stub core strategies");
 ...
 */
async function generateTemplateId$2({
  template
}) {
 ...
 */
async function verifyGeneratedTemplateId({
  template
}) {
 ...
 */
async function getInteractionTemplateAudits(context, {
  template,
  auditors
}, opts = {}) {
 ...
 * })
 */
async function generateDependencyPin({
  version,
  address,
  contractName
}, opts = {}) {
 ...
 * })
 */
async function generateDependencyPinAtLatestSealedBlock({
  version,
  address,
  contractName
}, opts = {}) {
 ...
 */
async function verifyDependencyPinsSame({
  template,
  blockHeight,
  network
}, opts = {}) {
      ...
      for (let templateDependencyPlaceholderKey of templateDependenciesPlaceholderKeys) {
        ...
        for (let templateDependencyPlaceholderContractName of templateDependencyPlaceholderContractNames) {
          ...
          const pin = await generateDependencyPin({
            ...
            contractName: templateDependency.contract}, opts);
          ...
          for (let k = 0; k < contract?.networks.length; k++) {
            ...
            if (net.network === network) {
              const pin = await generateDependencyPin({
                ...
                address: net.address,
                contractName: contract.contract}, opts);
 ...
 * @returns Whether or not the pins match
 */
async function verifyDependencyPinsSameAtLatestSealedBlock({
  template,
  network
}, opts = {}) {
  const latestSealedBlock = await block({
 ...
 * @returns Message
 */
function getTemplateArgumentMessage({
  localization = "en-US",
  argumentLabel,
  messageKey,
  template
...
function normalizeCompositeSignature(resp) {
  ...
  if (!resp["f_vsn"]) {
    return {
      ...
      keyId: resp.keyId,
      ...(resp.extensionData ? {
        extensionData: resp.extensionData
      } : {})
    };
  }
  switch (resp["f_vsn"]) {
 ...
 * })
 */
async function execService(context, {
  service,
  msg = {},
  config = {},
  opts = {},
 ...
 * @param config Current User Configuration
 */
const createResolvePreAuthz = context => (authz, {
  user
}) => {
 ...
 */
async function poll(service, checkCanContinue = () => true) {
 ...
 * })
 */
const getExecHttpPost = execLocal => async ({
  service,
  body,
  config,
  opts
...
export { index$1 as AppUtils, CORE_STRATEGIES, FCL_REDIRECT_URL_PARAM_NAME, FCL_RESPONSE_PARAM_NAME, index as InteractionTemplateUtils, TransactionError, URL$1 as URL, VERSION, index$2 as WalletUtils, buildMessageHandler, createFlowClientCore, discovery, events, execStrategy, getChainId, getCurrentUser, getExecHttpPost, getMutate, getServiceRegistry, initServiceRegistry, isReactNative, normalizePollingResponse, pluginRegistry, query, queryRaw, serialize, serviceEndpoint, setIsReactNative, transaction as tx, verifyUserSignatures };
